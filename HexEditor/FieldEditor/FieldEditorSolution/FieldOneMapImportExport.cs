...
partial class FieldViewerForm : Form
{
    private void oneMapExport_Click(Object sender, EventArgs e) {
        ...
        // 現在選択中のマップID
        int iSelectedMapID = FieldListComboBox.SelectedIndex;
                
        //SaveFileDialogクラスのインスタンスを作成
        SaveFileDialog sfd = new SaveFileDialog();

        //はじめのファイル名を指定する
        sfd.FileName = String.Format("Field{0:D3}", iSelectedMapID) + ".field";
        //はじめに表示されるフォルダを指定する
        sfd.InitialDirectory = System.IO.Directory.GetCurrentDirectory();
        //[ファイルの種類]に表示される選択肢を指定する
        sfd.Filter =
            "野戦データファイル(*.field)|*.field";
        //[ファイルの種類]ではじめに
        //「野戦データファイル」が選択されているようにする
        sfd.FilterIndex = 1;
        //タイトルを設定する
